import instances, pytesseract, torch, pygame, random, ffmpeg, playsound
...
class oldTTS():
    ...
    def voice(self, msg:str="TEST"):
        filename = f"C:/Users/kazen/Desktop/kazenoko/fishlang/kaz-ai/source/tts/TEMP{random.randrange(0, 65536)}.wav"
...
class tts():
    def __init__(self, model, volume:float=0.8):
        ...
        if self.model == "Sozomi":
            ...
            self.octave = -0.25
        elif self.model == "Afash":
            self.afashTTS = TTS("tts_models/de/thorsten/tacotron2-DDC")
        ...
        if message is None:
            try:
                with self.sr.Microphone() as self.source:
                    ...
                    return self.recogniser.recognize_whisper(audio_data=self.audio)
...
class AI():
    def __init__(self, username, model):
        ...
        elif self.mod == 2: self.model_name = "Afash"
